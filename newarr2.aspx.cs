...
public partial class newarr2 : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!Page.IsPostBack)
        {
            ...
            {
                Response.Redirect("newarr2.aspx?diff=newarr");
            }
            if (Session["username"] == null)
            {
                if (Request.Form["username"] != null)
                {
                    if (checkusername() == true)
                    {
                        ...
                        Response.Redirect("newarr2.aspx");
                    }
                    else
                    {
                        Response.Redirect("newarr.aspx?validlogin=false");
                    }
                }
            }
            else
    ...
    {
        Session.Abandon();
        Response.Redirect("newarr.aspx?signout=true");
    }
